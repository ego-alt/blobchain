...
class ST_random_prime:
    ...
    def find_c0(self):
        ...
        while self.prime_gen_counter <= (4 * self.length) and prime_no == 0:
            ...
            if isprime(c0):
                prime_no = c0
                print('SUCCESS 1', c0, self.prime_seed, self.prime_gen_counter)
                self.find_c(c0)
    ...
    def find_c(self, c0):
        """Generate a pseudorandom integer x in the interval [2 ** (self.length - 1) + 2 ** (self.length - 1)]"""
        x = 0
    ...
    def t_repeat(self, c0, x):
        """"Generate a candidate prime c in the interval [2 ** (self.length - 1) + 2 ** (self.length - 1)]"""
        z, c = 0, 1
        while 1 != gcd(z - 1, c) or 1 != (z ** c0) % c:
            ...
            """Test the candidate prime c for primality"""
            a = 0
...
prime = ST_random_prime(100, 9771)
